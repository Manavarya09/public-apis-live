...
export function renderReadme(data: ApiEntry[], date: string): string {
  ...
  const head = [
    ...
    "",
    `**${up} working public APIs · verified ${date} · refreshed daily** — [🔎 Search](https://manavarya09.github.io/public-apis-live/) · [📊 Benchmark](./BENCHMARK.md)`,
    "",
    ...
    "",
    `We check reachability daily (no API keys). A working API means its URL returned a success response (2xx/3xx) today; 📦 marks the ${dataVerified} no-auth APIs that also returned real data. We do not test auth-gated endpoints. ${down} unreachable and ${unknown} unverified entries are listed at the bottom.`,
    "</details>",
    "",
  ];
  const body: string[] = [];
